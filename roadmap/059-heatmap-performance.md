...
The heatmap visualization page (`/app/viz/<entity>`) is slow because it sidesteps every optimization shipped in [013-dashboard-performance.md](./013-dashboard-performance.md). For users with thousands of rows it still takes seconds to load, mostly waiting on network doc-fetches to the remote Neon doc store. This work brings the heatmap page to parity with the home timeline's perf characteristics and adds a better multi-year UX along the way.
...
- Timeline/Gantt charts (separate work unit: [016-generic-viz.md](./016-generic-viz.md) Phase 4)
...
**Reference docs:**
- [013-dashboard-performance.md](./013-dashboard-performance.md) — canonical pattern doc for scan-then-pull, not-clause removal, rel-cache batching (lines 187-228, 368-388, 390-407)
- [016-generic-viz.md](./016-generic-viz.md) — original viz system work unit (now shipped)
...
- **Verify on prod, not local.** Per 013-dashboard-performance.md, prod is XTDB `:jdbc` on Neon Postgres where doc pulls are network fetches — local won't show the win. Confirm via the perf dashboard that `viz-year-page` spans show index-only timings (no doc-store round trips).
