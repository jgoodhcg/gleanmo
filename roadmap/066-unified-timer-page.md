...
in `063-qol-quick-actions.md` item 1: instead of bypassing the `/app/timers` hub,
...
(`063-qol-quick-actions.md` item 2): the review step moves from forced-at-start
to opt-in-while-running.

### Stop returns to the issuing page

Extends item 2 of `063-qol-quick-actions.md`: the stop link
...
`029-search-filter.md`'s CRUD-list case can reuse it later. No Choices.js here —
this filters visible rows, it is not a form select.

### Navigation

- Sidebar (per `063-qol-quick-actions.md` item 1, amended): a single
...
(`036-timer-overlap-metrics.md`), exercise/boulder session screens (custom flows,
not timer-system entities), inline parent creation from the workspace
(`064-timer-dashboard-inline-create.md` — but note its empty-state case gets
...
- Sidebar/home integration points: `063-qol-quick-actions.md` items 1 and 5.
- Client-side filter precedent/consumer: `roadmap/029-search-filter.md`.
...
`063-qol-quick-actions.md` Notes. Supersedes that doc's original three-deep-link
sidebar treatment (amended 2026-07-27).

Implementation (2026-07-27): shipped in one pass together with
`063-qol-quick-actions.md` item 2 (stop redirect) and the item-1 sidebar timers
...
3. **Relocate cuts on one instant.** `relocate!` computes `(t/now)` once
   ...
   no gap, and nothing for `036-timer-overlap-metrics.md` to subtract.
...
relocate confirmation — written up as `067-global-action-modal.md` and left at
