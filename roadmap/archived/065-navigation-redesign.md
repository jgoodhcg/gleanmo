...
(`063-qol-quick-actions.md` item 1) shipped: the flat sidebar reorder is a good
...
1. **Layered navigation / information architecture.** Today every
   ...
   workspace — that layering already started with `066-unified-timer-page.md`).
...
2. **Consistent styling across all pages.** Pages were built at different
   ...
   timeline z-index bug in `008-backlog.md`). An audit + a small shared layout
...
- **`062-mobile-tab-bar.md` is absorbed into this work unit.** The bottom tab bar
...
- How much goes behind a command palette (`008-backlog.md` Generic Components)
...
- **Timeline day headings never stick** (`008-backlog.md`): the shell's
...
- `shared/mobile-tab-bar` — this absorbs `062-mobile-tab-bar.md`. The old mobile
...
- ~~Timeline day headings never stick~~ — **closed 2026-07-28 by removing the
  ...
  instead. See `008-backlog.md` — note that sticky remains unavailable app-wide,
