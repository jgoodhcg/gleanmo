...
> **Archived 2026-02-21**: Pain tracking unified into `symptom-log` schema with `:pain` and `:chronic-pain` types. Airtable pain data will be ported as `symptom-log` records with `:type :pain`. See `014-data-migration-status.md` for migration tracking.
