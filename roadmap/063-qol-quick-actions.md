...
> the workspace** (`066-unified-timer-page.md`). The sidebar gets one timers
> link instead.
...
- **`⏱ timers → /app/timers`** first — the unified timer workspace
  (`066-unified-timer-page.md`): all running timers across types,
  search-to-start.
...
`066-unified-timer-page.md` extends the same principle to **start** (direct POST,
...
### 3. Execute `048-inline-entity-creation.md` Phases 1–2
...
- [ ] Item 3 has its own checklist in `048-inline-entity-creation.md`.

## Scope

**Out of scope** (documented elsewhere, deliberately not here):

- Usage-weighted/adaptive quick-action ordering — `035-workflow-optimization.md`.
  This work unit ships a static order from a known 28-day sample.
- "Repeat last" one-tap logging — `035-workflow-optimization.md`.
- Mobile bottom tab bar — `archived/062-mobile-tab-bar.md`.
- Command palette — `008-backlog.md` (Generic Components).
...
specced separately in `066-unified-timer-page.md` — same batch, own validation
checklist.
...
- Analytics source: Plausible CSV export
  ...
  execute.
- Related vision doc: `035-workflow-optimization.md` (this work unit is its first
...
of m003–m006, then Airtable retirement (see `014-data-migration-status.md`).
...
2 (touches e2e), then `066-unified-timer-page.md` (builds on 2's redirect param),
...
`066-unified-timer-page.md` implementation; e2e timer-stop flow updated there.
