...
5. **Tab order itself is fine** — DOM order matches visual order (single-column grid), Cancel precedes Save in edit forms, Delete is last. No `tabindex` juggling needed (see [044-form-tab-ordering.md](./044-form-tab-ordering.md), which this supersedes/absorbs).
...
- Supersedes the tab-ordering concern in [044-form-tab-ordering.md](./044-form-tab-ordering.md): audit found DOM order already matches visual order; the real gaps are label association, focus visibility, and hover-only controls.
