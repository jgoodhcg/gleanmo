...
(deftest query-count-with-relationship-filtering-test
  (testing "query count when filtering by related entity attributes"
    (with-open [node (test-xtdb-node [])]
      (let [ctx (get-context node)
            user-id (UUID/randomUUID)
            _ (dotimes [_ 5]
                (create-related-entities ctx user-id))
            db (xt/db node)
            relationship-fields [{:field-key :cruddy/single-relation
                                  :input-type :single-relationship
                                  :related-entity-str "habit"}
                                 ...
                                 {:field-key :cruddy/set-relation
                                  :input-type :many-relationship
                                  :related-entity-str "habit"}]]
        (testing "uses batched queries for related entity lookups"
          ;; With batching: O(1) network round-trips (1 main + 1 batch query)
          ;; Without batching: O(N*M*K) round-trips where N=entities, M=fields, K=IDs
          ;; Data processing is still O(n), but network latency dominates
